...
namespace ashan
{
	...
	void rediscli::__addread(void *privdata)
	{
		...
		LOG_DBG("fd:%d events:%x", _this->m_fd, _this->m_events);
	}
	...
	{
		auto _this = (rediscli *)(privdata);
		LOG_DBG("%p fd:%d events:%x", _this, _this->m_fd, _this->m_events);
		_this->__clear();
	}
	...
	int rediscli::query(std::string &&_query, rediscb &&_callback)
	{
	...
	void rediscli::__oncallback(redisReply *_reply)
	{
		...
		if (_reply != nullptr)
		{
			...
			case REDIS_REPLY_NIL:
				r.last_error = STRING_NIL;
				break;
			case REDIS_REPLY_ERROR:
				r.last_error = _reply->str;
				break;
