...
namespace ashan
{
	...
	mysqlcli::mysqlcli(const std::string &url)
	{
	...
	int mysqlcli::query(std::string &&_sql, mysqlcb &&_cb)
	{
	...
	int mysqlcli::__tryconnect()
	{
		...
		LOG_DBG("mysql_real_connect_nonblocking fd:%d status:%d addr:%s:%d %s %s %s",
	...
	int mysqlcli::__onquery()
	{
		if (auto it = m_list.begin(); it != m_list.end())
		{
			...
			LOG_DBG("query async(%d):%s %s", status, _sql.c_str(), mysql_error(m_mysql));
			return status;
		}
