...
namespace ashan
{
	...
	void asyncnet::__emplace_back(std::vector<fcb> &vec, int fd, fcb &&obj)
	{
		...
		vec[fd] = std::move(obj);
		LOG_CORE("thdid:%d fd:%d", g_thdid, fd);
		assert(vec[fd] != nullptr);
	}
	...
	int asyncnet::__wait()
	{
		...
		{
			int fd = ((__events[i].data.u64) & 0xffffffff);
			int state = ((__events[i].data.u64) >> 32);
			LOG_CORE("epoll_wait thdid:%d fd:%d state:%d", g_thdid, fd, state);
			__TRY()
			switch (state)
			{
			case e_state_client:
	...
	int asyncnet::__call(std::vector<fcb> &_vec, int fd, int __fd)
	{
		...
		if (_vec[fd])
		{
			LOG_CORE("thdid:%d fd:%d:%d", g_thdid, fd, __fd);
			_vec[fd](this, __fd);
		}
		return 0;
	}
	...
	int asyncnet::__onclient(int fd, int __events)
	{
		if (__events & EPOLLIN)
		{
			LOG_CORE("client read thdid:%d fd:%d", g_thdid, fd);
			__call(m_read, fd);
		}
		if (__events & EPOLLOUT)
		{
			LOG_CORE("client send thdid:%d fd:%d", g_thdid, fd);
			__call(m_write, fd);
		}
		if (__events & (EPOLLERR | EPOLLHUP))
		{
			LOG_CORE("client close %d %s", fd, m_flags[e_flag_close] ? "true" : "false");
			if (m_flags[e_flag_close])
			{
				__clientclose(fd);
			}
